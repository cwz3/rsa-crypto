...
;
;(define result-2
;     (rsa-encrypt "Who is Dr. Racket?" (key-pair-public test-key-2)))
;
;(rsa-decrypt result-2 (key-pair-private test-key-2))
;
;(define result-3
;     (rsa-encrypt "Jerry is thebomb.com" (key-pair-public test-key-1)))
;
;(rsa-decrypt result-3 (key-pair-private test-key-1))


;problem 2

(define encrypt-and-sign
  (lambda ((msg <string>) (pub <key>) (priv <key>))
    (make-signed-message (rsa-encrypt msg pub) (rsa-transform (compress (rsa-encrypt msg pub) pub) priv))))
...
(define decrypt-and-verify
  (lambda ((signed <list>)(priv <key>) (pub <key>))
    (if (= (rsa-transform (cdr signed) pub) (compress (car signed) priv))
          (rsa-decrypt (car signed) priv)
          #f)))
...
(define crack-rsa
  (lambda ((pub <key>))
    (let* ((p (smallest-divisor (key-modulus pub)))
          (q (/ (key-modulus pub) p))
          (m (* (- p 1) (- q 1)))
          (d (modulo(car (euclid (key-exponent pub) m)) m)))
      (make-key d (key-modulus pub)))))
...
;Sender = Ben
;Recipient = Andy
;Message : "If cryptography is outlawed, only outlaws will have cryptography."
...
(define pollard
  (lambda ((n <integer>) (base <integer>) (max <integer>))
       (letrec ((iter
                (lambda (k r num b m)    
                  (let* ((rnew  (exptmod r k n) )
                              (d (car (cddr (euclid (- rnew 1) num)))))
                              (cond ((> k m) #f)
                                    ((= d 1)
                                     (iter (+ k 1) rnew num b m))
                                    ((= d num)
                                     (iter (+ k 1) rnew num b m))
                                    (else
                                     d))))
    ...
    (iter 1 (modulo base n) n base max ))))
